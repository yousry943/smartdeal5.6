        ...
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <div class="box box-info">
                    ...
                    <form class="form-horizontal" method="post" enctype="multipart/form-data"  action="{{url(LaravelLocalization::setLocale().'/user/technical_analysis/'.$news->first()->id)}}">
                      {{csrf_field()}}
...
<option value="{{$news->first()->lang_id}}">{{$news->first()->get_language()->first()->name}}</option>
                                         @foreach($languages as $language)
                                                 <option value="{{$language->first()->id}}">{{$language->first()->name}}</option>
                                         @endforeach
                                     </select>
                                 </div>
                             </div>
